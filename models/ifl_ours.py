...
def DesignedEnsembleDisLabeled(train_datasets, test_datasets, word2idx, alpha=0.2, beta=0.2, gamma=5, epochs=150, batch_size=64, learning_rate=0.005, weight_decay=0.1, source_embeddings=None, total_tasks=3, device=device, T=2, metrics='loss', early_stop_threshold=10):
    ...
    def Ensemble_P(shared_output, specific_output, T):
        # p = (torch.softmax(shared_output, dim=1) + k * torch.softmax(specific_output, dim=1)) / (1 + k)
    ...
    classifier_params = {'num_specific_features': 8, 'num_specific_classes': 2}
    ...
    for task in range(total_tasks):
        ...
        shared_classifier_optimizer = torch.optim.Adam(shared_classifier.parameters(), lr=learning_rate, weight_decay=weight_decay)
        ...
        for epoch in range(epochs):            
            ...
            for i, (data, label) in enumerate(train_dataloaders[task]):
                ...
                shared_output = shared_classifier(mid_shared_output)
                ...
                label_p = [-nn.NLLLoss(reduction='none')(p, label) for p in added_p]
                ...
                if task > 0:
                    for old_task in range(task):                        
                        specific_output = classifiers[old_task](mid_specific_outputs[old_task])
                        # temp_specific_output = Variable(specific_output.data, requires_grad=False).to(device)
                        norm_q = Ensemble_P(shared_output, specific_output, T=T)
                        new_output = torch.log(norm_q)
                        old_output = Ensemble_P(old_shared_output, specific_output, T=T)
        ...
        for t in range(task + 1):
            ...
            for i, (data, label) in enumerate(test_dataloaders[t]):
                ...
                # output = (shared_output + k * specific_output) / (1 + k)
                accuracy_list.append(AccuarcyCompute(output, label))
                # accuracy_list.append(F1Compute(output, label))
    ...
    print('===========================================================================')
    with open('results/'+str(today)+'_RecData_EnsembleDisLabeled_ParamSearch.txt', 'a+') as f:
